...
$ python3 ask_probability.py <正解率> <問題数> <正解数> <ラージデータモード>
```
で実行可能。＜正解率＞は、解説前の正解率である。例えば25％であった場合、0.25 というように入力する。＜問題数＞は全体の問題数である。＜正解数＞は、解説後の正解数である。＜ラージデータモード＞は、問題数が大きくなり計算途中でオーバーフローを発生するのを防ぐモードである。"ON"や"OFF"で切り替えできる。下記でその中身を説明する。

### デモ
```bash
$ python3 ask_probability.py 0.3 60 25 OFF
```
結果は、
```bash
number of correct answer -> 25   probability ->  0.03623751946731613
```
と返ってくる。"number of correct answer"が、入力した正解数。"probability"が、その正解数以上が発生する確率である。
